...
class ParseSpecial:
    ...
    RULES = [
        {
            ...
            'fields': {
                'date': {
                    'element': lambda e: e.name == 'a' and e['href'].startswith('/schedules') and e.findChildren('span', {'class', 'time'}),
                    'value': lambda e: str(e.contents[0]).split(',', 1)[-1].strip(),
                },
                'type': {
                    'element': lambda e: e.name == 'aside' and 'block' in e.get('class', []) and e.findChildren('img'),
                    'value': lambda e: 'radioBroadcast' if 'radio' in e.text else 'tvBroadcast',
                },
            },
        },
    ]
    ...
    def apply(self, citation: Citation) -> Citation:
        ...
        matchedRules = [rule for rule in ParseSpecial.RULES if self._matchCitation(citation, rule['match'])]
